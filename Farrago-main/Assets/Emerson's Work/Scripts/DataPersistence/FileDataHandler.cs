...
public class FileDataHandler
{
    ...
    public CareerData LoadCareer(string profileId, bool allowRestoreFromBackup = true) 
    {
        // base case - if the profileId is null, return right away
        if (profileId == null) 
        {
        ...
        if (File.Exists(fullPath)) 
        {
            try 
            {
                ...
                // deserialize the data from Json back into the C# object
                loadedData = JsonUtility.FromJson<CareerData>(dataToLoad);
            }
            catch (Exception e) 
            {
                // since we're calling Load(..) recursively, we need to account for the case where
                ...
                if (allowRestoreFromBackup) 
                {
                    ...
                    {
                        // try to load again recursively
                        loadedData = LoadCareer(profileId, false);
                    }
                }
                // if we hit this else block, one possibility is that the backup file is also corrupt
                else 
    ...
    public void Save(CareerData data, string profileId) 
    {
        // base case - if the profileId is null, return right away
        if (profileId == null) 
        {
        ...
        try 
        {
            ...
            // verify the newly saved file can be loaded successfully
            CareerData verifiedCareerData = LoadCareer(profileId);
            // if the data can be verified, back it up
            if (verifiedCareerData != null) 
            {
                File.Copy(fullPath, backupFilePath, true);
            }
            // otherwise, something went wrong and we should throw an exception
